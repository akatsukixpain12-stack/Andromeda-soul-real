...
export const AndromedaNavbar: React.FC<AndromedaNavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200/70 px-3 sm:px-6 py-2.5 shrink-0">
      <div className="w-full max-w-[1440px] mx-auto flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          >
            <img
              ...
              className="w-5 h-5 rounded-md object-cover border border-indigo-200/60"
            ...
            {isEditingTitle ? (
              <form onSubmit={handleSaveTitle} className="flex items-center gap-1">
                <input
                  ...
                  autoFocus
                  className="px-2.5 py-0.5 text-xs sm:text-sm font-medium bg-white border border-indigo-400 rounded-lg text-slate-900 outline-none shadow-xs"
                />
              </form>
            ) : (
              <button
                onClick={handleStartEditing}
                className="group flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-slate-800 truncate hover:text-indigo-600 transition-colors"
                title="Click to rename conversation"
              >
                <span className="truncate">{activeConversationTitle || 'New Conversation'}</span>
                <Edit2 className="w-3 h-3 opacity-0 group-hover:opacity-100 text-slate-400 transition-opacity shrink-0" />
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-900 border border-emerald-200/80 hover:bg-emerald-100/80 transition-all cursor-pointer shadow-2xs bouncy-btn"
            title="Google Cloud Firestore: All chats & autonomous learned memory synced to Google Cloud Server. Click to view."
          >
            ...
            <span className="text-[10px] text-emerald-800 bg-emerald-200/70 font-mono px-1.5 py-0.5 rounded-md">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-slate-700 hover:text-slate-900 hover:bg-slate-100/80 border border-slate-200/80 transition-all cursor-pointer bouncy-btn"
            title="Configure API Keys & Fleet"
          >
          ...
          <button
            ...
            onClick={onOpenAuth}
            className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer border bouncy-btn ${
            ...
            {currentUser && currentUser.provider === 'google' ? (
              <>
                <UserAvatar
                  ...
                  size="xs"
                />
                <span className="hidden sm:inline max-w-[100px] truncate font-semibold text-slate-900">
          ...
          <button
            ...
            onClick={onNewChat}
            className="flex items-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-1.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-md shadow-indigo-100 cursor-pointer shrink-0 bouncy-btn"
